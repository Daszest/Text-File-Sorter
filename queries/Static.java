...
public class Static extends Query
{
	...
	// Creates a Static (not the java keyword) object
	public Static(String fileID, String queryID)
	{
	...
	// Default constructor for a Static (not the java keyword) object
	public Static()
	{	
	}
	...
	// A method that does all the heavy lifting for the Static query
	// The Static query is supposed to list all source modules declared in the text file 
	// Along with the number of other modules from which the source module uses static members (methods or fields)
	// The order of the list of source modules is the natural ordering of "Module" objects (using "compareTo" method in class "Module")
	// This method does all the behind scenes work for Static, that is it does everything except for printing the information to the console and the sorting
	...
	public void inquire(String QueryCommand)
	{
		...
		// 3:	The source module uses a static member in the target dependency
		...
		// Second by comparing the "Uses" part of the data to the String "Static"
		// If the "Uses" part of the data ends with "Static", then the source module uses a static member in the target dependency
		...
		if(!_srcSet.add(moduleName()))
		{
			if(moduleHasATarget() && !moduleDependsOnItself() && moduleCategory().length() >= 6)
			{
				if(moduleCategory().substring(endInd - 6, endInd).equals("Static"))
				{
					if(_staticSet.add(tgtConcat))
					{
	...
	// A method that prints the output of the "Static" query to the command line
	public void output()
	{
